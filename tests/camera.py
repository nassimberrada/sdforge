...
def test_camera_instantiation_custom_values():
    ...
    cam = Camera(position=pos, target=tgt, zoom=zoom, type=cam_type)
    
    assert cam.position == pos
    assert cam.target == tgt
    assert cam.zoom == zoom
    assert cam.type == cam_type
...
def test_camera_isometric_creation():
    """Tests specifically that an isometric camera can be cleanly created by just setting type."""
    cam = Camera(type='orthographic')
    
    # Defaults should be preserved to naturally form the isometric viewing angle
    assert cam.position == (5, 4, 5)
    assert cam.target == (0, 0, 0)
    assert cam.type == 'orthographic'