...
class Box(SDFObject):
    ...
    def __init__(self, size=1.0, radius=0.0, x=None, y=None, z=None):
        super().__init__()
    ...
    def to_callable(self):
        is_animated = any(isinstance(v, str) for v in self.size) or isinstance(self.radius, str)
        if is_animated:
        ...
        def _callable(p):
            ...
            dist = np.linalg.norm(np.maximum(q, 0), axis=-1)
            if self.radius == 0.0:
                dist += np.minimum(np.max(q, axis=-1), 0)
            else:
                dist -= self.radius
            return dist
        return _callable

def box(size=1.0, radius=0.0, x=None, y=None, z=None) -> SDFObject:
    ...
    Args:
        size (float, tuple, or str, optional): The size of the box along each axis.
                                               ...
                                               This is ignored if x, y, z are provided.
        radius (float or str, optional): The radius of the rounded edges. Defaults to 0.0.
        x (float, optional): Size on the X axis.
        y (float, optional): Size on the Y axis.
        z (float, optional): Size on the Z axis.
    """
    return Box(size, radius, x, y, z)
...
class Line(SDFObject):
    """Represents a line segment primitive with a radius."""
    def __init__(self, a, b, radius=0.1, rounded_caps=True):
        super().__init__()
    ...
    def to_glsl(self) -> str:
        ...
        return f"vec4({func}(p, {a_str}, {b_str}, {r}), -1.0, 0.0, 0.0)"
    def to_callable(self):
...
def line(a, b, radius=0.1, rounded_caps=True) -> SDFObject:
    """Creates a line segment with a given radius.

    Args:
        a (tuple or np.ndarray): The start point of the line.
        b (tuple or np.ndarray): The end point of the line.
        radius (float or str, optional): The radius of the line. Defaults to 0.1.
        rounded_caps (bool, optional): If True, creates a capsule with rounded ends.
                                        If False, creates a cylinder with flat ends. Defaults to True.
    """
    return Line(a, b, radius, rounded_caps)

class Cylinder(SDFObject):
    """Represents a cylinder primitive, possibly with rounded edges."""
    def __init__(self, radius=0.5, height=1.0, round_radius=0.0):
        super().__init__()
    ...
    def to_glsl(self) -> str:
        if self.round_radius > 0:
            ...
            r = _glsl_format(self.radius)
            h_half = _glsl_format(self.height / 2.0) if not isinstance(self.height, str) else f"({self.height})/2.0"
            return f"vec4(sdCylinder(p, vec2({r}, {h_half})), -1.0, 0.0, 0.0)"
    def to_callable(self):
        if isinstance(self.radius, str) or isinstance(self.height, str) or isinstance(self.round_radius, str):
            raise TypeError("Cannot save mesh of an object with animated (string) parameters.")
        if self.round_radius > 0:
            ra, rb, h = self.radius, self.round_radius, self.height
            def _callable_rounded(p):
...
def cylinder(radius=0.5, height=1.0, round_radius=0.0) -> SDFObject:
    ...
    Args:
        ...
        round_radius (float, optional): The radius of the rounding on the edges. Defaults to 0.0.
    """
...
class Cone(SDFObject):
    ...
    def to_glsl(self) -> str:
        ...
        # Use the simpler cone sdf if it's a true cone
        return f"vec4(sdCone(p, vec2({h}, {r1})), -1.0, 0.0, 0.0)"
    def to_callable(self):
...
def cone(height=1.0, radius1=0.5, radius2=0.0) -> SDFObject:
    ...
    Args:
        ...
        radius1 (float or str, optional): The radius of the base at Y=0. Defaults to 0.5.
        radius2 (float or str, optional): The radius of the top at Y=height.
                                           If 0, a standard cone is created. Defaults to 0.0.
    """
...
class Ellipsoid(SDFObject):
    ...
    def __init__(self, radii=(1, 0.5, 0.5), x=None, y=None, z=None):
        super().__init__()
        if x is not None and y is not None and z is not None:
            radii = (x, y, z)
        self.radii = radii
...
def ellipsoid(radii=(1, 0.5, 0.5), x=None, y=None, z=None) -> SDFObject:
    """Creates an ellipsoid.

    Args:
        radii (tuple or list): The radii of the ellipsoid along the X, Y, and Z axes.
                               Ignored if x, y, z are provided.
        x (float, optional): Radius on the X axis.
        y (float, optional): Radius on the Y axis.
        z (float, optional): Radius on the Z axis.
    """
    return Ellipsoid(radii, x, y, z)
