...
from sdforge.api.primitives import Box # <-- Import the Box class
...
def test_align_to_face_cardinal_axes():
    """Tests that align_to_face produces the correct sequence of transforms."""
    c = cylinder(radius=0.1, height=1.0)
    origin_point = np.array([0,0,0])
    
    # Align to +X face
    aligned_x = align_to_face(c, reference_point=origin_point, face_normal=X)
    ...
    aligned_y = align_to_face(c, reference_point=origin_point, face_normal=Y)
    ...
    # Align to -Z face
    aligned_neg_z = align_to_face(c, reference_point=origin_point, face_normal=-Z)
...
def test_align_to_face_with_offset():
    c = cylinder(radius=0.1, height=1.0)
    origin_point = np.array([0,0,0])
    aligned_x = align_to_face(c, reference_point=origin_point, face_normal=X, offset=2.0)
    
    # Should be at half-height (0.5) + offset (2.0)
    assert np.allclose(aligned_x.offset, [2.5, 0, 0])

def test_align_to_face_non_cardinal_axis_fails():
    c = cylinder(radius=0.1, height=1.0)
    with pytest.raises(ValueError, match="only supports cardinal axis normals"):
        align_to_face(c, reference_point=np.array([0,0,0]), face_normal=np.array([1,1,0]))

def test_place_at_angle():
    ...
    placed_obj = place_at_angle(b, pivot, axis=Y, angle_rad=angle, distance=distance)
...
def test_offset_along():
    """Tests the offset_along helper."""
    b = box(0.1)
    start_point = np.array([1, 2, 3])
    direction = np.array([0, 1, 0]) # Y-axis
    distance = 10.0
    
    offset_obj = offset_along(b, start_point, direction, distance)
...
def test_bounding_box():
    """Tests the bounding_box helper using a mock."""
    s = sphere(1.5)
    ...
    with patch.object(s, 'estimate_bounds', return_value=mock_bounds) as mock_method:
        bbox = bounding_box(s, padding=0.1)
        ...
        assert isinstance(bbox.child, Box) # <-- FIX: Check against the class `Box`, not the function `box`
