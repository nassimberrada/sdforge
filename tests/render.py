...
from sdforge.api.engine.render import NativeRenderer
from sdforge.api.primitives.geometry import Sphere 
...
@patch('sdforge.api.engine.render.NativeRenderer._compile_shader')
...
@patch('sdforge.api.engine.mesh.generate')
...
@patch('sdforge.api.engine.render.NativeRenderer.run')
...
def test_render_mode_auto_detects_desktop(mock_render_mesh, mock_run):
    ...
    s = sphere()
    with patch('sdforge.api.engine.render._IS_NOTEBOOK', False):
        with patch.dict('sys.modules', {'moderngl': MagicMock(), 'glfw': MagicMock()}):
            s.render(mode='auto')
