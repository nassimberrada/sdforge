...
def main():
    ...
    tree_top = cone(height=1.5, radius1=0.8) + Y * 1.5
    ...
    # An octahedron "gem"
    gem = octahedron(0.5) + (-X * 2 + Y * 0.5)

    # A rounded box "pedestal" for the gem
    pedestal = box(size=(0.7, 0.2, 0.7), radius=0.05) + (-X * 2 + Y * 0.1)

    # A hex prism "nut"
    nut = hex_prism(radius=0.4, height=0.2) + (X * 2 + Y * 0.2)
    # Cut a hole through the nut
    nut -= cylinder(0.2)

    # An ellipsoid "stone"
    stone = ellipsoid(x=0.8, y=0.4, z=0.5) + (Y * 0.4 + X * 3.5)

    # A pyramid
    pyr = pyramid(height=1.0) + (-X * 4 + Y * 0.5)

    # A rounded cone
    rc = round_cone(radius1=0.6, radius2=0.2, height=1.2) + (-X * 5.5 + Y * 0.6)

    # A box frame
    bf = box_frame(size=(1, 1, 1), edge_radius=0.1) + (X * 5.5 + Y * 0.6)
