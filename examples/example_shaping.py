...
def main():
    ...
    # --- Extrusion ---
    # To extrude, we first need a 2D SDF. We can define one using `Forge`
    # or by creating a custom SDFObject that only uses p.xy.
    # Here, we use Forge to create a 2D flower shape.
    flower_2d = Forge("""
        vec2 q = p.xy;
        q = abs(q);
        float a = atan(q.y,q.x);
        float r = length(q);
        return r - (0.8 + 0.2*sin(a*5.0));
    """)
    
    extruded_shape = flower_2d.extrude(0.2).translate(-X * 4)
    ...
    final_scene = displaced_shape | extruded_shape | repeated_obj
